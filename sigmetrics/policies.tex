...
\end{equation}
where $P(t)$ is the probability of a preemption at time $t$, and is given by $P(t) = f(t)$ , where $f(t)$ is the probability distribution function given by Equation~\ref{eq:failrate}.
...
\begin{align}
  ...
  &= (1-F(T))~T + F(T)~T + \int_0^{T} t~f(t)~dt \\ \nonumber
       &= T + \int_0^{T} t~f(t)~dt         
\end{align}
This expression for the expected running time assumes that the job will be preempted at most once and ignores the higher order terms.
Since the probability of multiple preemptions is low, most transient computing systems seek to avoid repeated preemptions, and discard the job if multiple preemptions occur or move them to on-demand VMs. 
...
Users and transient computing systems can thus use the expected running time analysis for scheduling and monitoring purposes.
...
Intuitively, the checkpointing rate should depend on the failure rate, and our analytical preemption model can be used for designing an optimized checkpointing schedule. We now present our checkpointing policy that uses the preemption model, and results in non-uniform checkpoint intervals:  
...
Let the checkpoint cost be $\delta$---i.e, it takes each checkpoint increases the running time by $\delta$. 
...
Let $M(W, t)$ denote the makespan where $W$ is remaining length of job to be executed, and $t$ is amount of time already elapsed. %, and $s$ was the starting time (wallclock time) of the job.
...
Once all the values of $E[M(W, t, i)]$ are computed, the job's checkpointing schedule is determined as follows.
We first locate the checkpointing interval $i_1$ that minimizes $E[M(W,0,i]$.
Then, we recursively find the next checkpointing interval $i_2$ by minimizing  $E[M(W-i_i,i_i,i)]$, and so on, until the $W\leq0$.

If a job encounters a failure, it is resumed from the most recent checkpoint, on a new VM.
After every such resume-event, we compute the optimal checkpointing schedule again, since the job's failure rate is dependent on the VM age when it starts, and the job may be resumed at a later time or on a VM of a different type, etc.
