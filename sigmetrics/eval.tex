...
\prat{Show Scispot selection and naive selection}
...
\sysname provides the option to alleviate the parallel scaling bottleneck, by increasing the number of parallel jobs.
That is, for a given fixed cost, it can run more jobs in parallel, by reducing the total resources allocated and hence the parallelism 
...
For both Nanoconfinement and Shapes, we see up to $6\times$ reduction in the total bag running time, when more number of jobs are launched in parallel (with a smaller number of CPUs per job). 
