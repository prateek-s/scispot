...
%\vspace*{\subsecspace}
\begin{comment}
\noindent \textbf{Cloud Computing For Science}
...
% However, it remains to be seen how Amazon's recent change~\cite{bid-change, irwin-icccn19, baughman2019deconstructing} in the preemption model of spot instances affects prior work.
In contrast, non-price based transient availability models, such as temporally constrained preemptions, have received scant attention because of the difficulty in obtaining empirical preemption data---which we hope our dataset remedies. 

%principled and optimized use of transient resources provided by other cloud providers such as Google, Microsoft, Packet, and Alibaba is still nascent, and ours represents an initial step. 
%
%
%Ours is the first work that studies the preemption characteristics and addresses the challenges involved in running large-scale applications on the temporally constrained  transient VMs, and provides insights on the unique constrained preemption dynamics. %, as explained in Section~\ref{sec:preemption-dynamics}.
...
%, based on older spot pricing models. 
...
%Explain more how the preemptions happen. Exosphere? SpotOn batch service. 
