...
\vspace*{\subsecspace}
\section{Modeling Constrained Preemptions}
...
>>>>>>> 31b43df25a581118942f642f4aca70edce0edf93
\vspace*{\subsecspace}
...
\prat{Hazard rate function}

For Google Premptible VMs, $S (t) = e^{- r_1 t} - e^{r_2 (t - b)}$

and $f (t) = - r_1 e^{- r_1 t} - r_2 e^{r_2 (t - b)}$

Thus we compute $\lambda = \dfrac{- r_1 e^{- r_1 t} - r_2 e^{r_2 (t -
b)}}{e^{- r_1 t} - e^{r_2 (t - b)}}$
=======
...
<<<<<<< HEAD
It may be useful to think of $e^{- r_2 b}$ as the ``damping factor'' for the
...
\prat{Comparison with other distributions. Makeham Gompertz, etc}

\prat{Goodness of fit for each distribution. QQ plots}

Even though it may be possible for other distributions to fit this, it is more important to capture the distinct phases, which is what our CDF does well. 
