...
%
Preemptions behavior in temporally constrained systems is \emph{not} determined by dynamic pricing, and prior pricing based approaches developed for spot markets are not applicable.
...
%
Our job-scheduling policy uses the the bathtub behavior to decide whether to run a new job on a running VM or to request a new VM, and reduces job-failure probability by $5\times$ compared to existing memoryless preemption models.
%
...
%
Our checkpointing policy is based on dynamic programming, and can reduce the checkpointing overhead by more than $10\times$. 
%
