...
For the Nanoconfinement application, we observe that the the base running times (without preemptions) reduce when moving to larger VMs, because this entails lower communication costs.
...
We define the overhead as the increase in running time which is equal to the turnaround time (that is the time between the user submitting the job and getting the results back) divided by the base job running time (with no waiting or premptions). 
...
In all cases unless other wise stated, we run bags of 36 jobs, and impose that 90\% of all jobs complete (thus we target a completion of 32 jobs).
...
It is common to deploy scientific applications on large clusters, and we evaluate \sysname on different cluster sizes in Figure~\ref{fig:vm-per-job-scaling}.
The figure shows the total running time of the bag of jobs for the NC and Shapes applications as the total number of VMs (and hence total number of CPUs) increases.
...
Since \sysname reuses VMs when running jobs from a bag, it is able to take advantage of the relatively low preemption rates of VMs once they pass the first phase of early failures (Figure~\ref{fig:cdf}), and thus minimizes the number of preemptions and thus job failures. 
This makes \sysname particularly suitable for running the large bags of jobs that are required when using machine learning techniques for HPC workloads~\cite{}, since the training and testing data needed for statistical machine learning can be generated through \sysname's bag of jobs execution model. 
...
Figure~\ref{fig:fails-time} shows the running time of the bag of 32 NC jobs on a cluster of 4 \texttt{n1-highcpu-32} VMs, when different number of VMs are preempted. 
