...
A sample of over 100 such preemption events are shown in Figure~\ref{fig:gcp1}, which shows cumulative distribution of the VM lifetimes. \vj{these samples are taken from different VM types and normalized? could be useful to discuss this detail here}
...
Our new model is based on the earlier observation that the cumulative distribution of lifetimes has multiple distinct temporal phases. The key assumption underlying our minimal model is the presence of two distinct failure processes.
...
\end{equation}
$p(t)$ vs. $t$ yields a bathtub type failure rate function for typical fit parameters as shown in the inset of Figure~\ref{fig:gcp1}.
...
Figure~\ref{fig:cdf-comparison} shows the premption data from five different types of VMs in the Google Cloud \texttt{n1-highcpu-\{2,4,8,16,32\}}, where the number indicates the number of CPUs. We also show the associated preemption CDFs ($\mathscr{F}$)computed using the proposed model. %along with the parameters of our four parameter distribution.
...
\end{equation}
where $p_i(t)$ is the rate of preemptions of VMs of type $i$ given by Eq.~\ref{eq:failrate}.
