...
\subsection{Price\vj{-}based preemption models}

Amazon's EC2 spot instances were the original cloud transient servers.
The preemptions of EC2 spot instances \vj{\sout{is}are} \vj{\sout{based on}controlled by} their \emph{price}, which is dynamically adjusted based on the supply and demand of cloud resources.
Spot prices are \vj{\sout{determined}} based on a continuous second-price auction, and if the spot price increases above a pre-specified maximum-price, then the server is preempted.
...
\vj{\subsection{Empirical models of preemptions}}
...
In such cases, price based models are inadequate, and other approaches to understand preemptions \vj{\sout{is}are} required.
...
We observe that many VMs are quickly preempted after they are launched, and thus have a steep rate of failure initially before the rate stabilizes (and the CDF approaches a saturated region).
...
We note that this preemption behavior, imposed by the \vj{constraint of the} small, 24 hour  lifetime, is \emph{substantially} different from conventional failure characteristics of hardware components and even EC2 spot instances.
In these ``classical'' setups, the rate of failure usually follows an exponential distribution, such that $f(t) = \lambda e^{-\lambda t}$, where $\lambda=1/\text{MTTF}$.
...
This new model is based on the earlier observation that the distribution of cumulative lifetimes has multiple distinct temporal phases. The first two phases are reasonably captured by the classic exponential distribution. 
In order to model the overall observed empirical CDF, we add another term that captures the failure process near the 24 hour maximum VM lifetime,  and construct a \emph{new} probability distribution. 
%
...
We write the general form of the CDF associated with the proposed probability distribution as follows:
...
For most of it's life, a VM sees failures according to the convential exponential distribution with a rate of failure equal to $1/\tau_1$ -- this behavior is captured by $1-e^{-t/\tau_1}$ term in Eq.~\ref{eq:blend1}. 
