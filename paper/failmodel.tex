...
A sample of \vj{100 check} such preemption events are shown in Figure~\ref{fig:gcp1}, which shows cumulative distribution of the VM lifetimes. \vj{these samples are taken from different VM types and normalized? could be useful to discuss this detail here}
Note that the cloud operator (Google) caps the \emph{maximum} lifetime of the VM to 24 hours, and in an ideal scenario all the VMs are preempted before that hard limit. \vj{this limit is not that hard, but a bit elastic, given that our x-axis goes to 25 and there is data beyond 24. i added ideally to reflect this}
