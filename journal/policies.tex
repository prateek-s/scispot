...
\vikram{the following para probably is still in the works?}
\noindent \textbf{Transition Points Analysis.}
Our model can also be used to find the transition boundary in the job scheduling policy. 
The decision to reuse or launch a new VM depends
For a given job running time (T), and the VM age (s), we can compute  $\delta(s,T) = E[T_s] - E[T]$.
For a given VM type, we precompute this function for different $s,T$ values, and find the job length trasition points where launching a new VM is suitable.

\begin{figure}[t]
  \subfloat[Difference in expected running times. \label{fig:transition-all}]
  {\includegraphics[width=0.3\textwidth]{../graphs/transition-all.pdf}}
  \hfill
  \subfloat[Transition boundary. \label{fig:transition-boundary}]
  {\includegraphics[width=0.18\textwidth]{../graphs/tipping.pdf}}
...
%
Since server selection involves a tradeoff between cost, performance, and preemptions, we develop a model that allows us to optimize the resource allocation and pick the best VM type that minimizes the expected cost of running an application on \sysname. \vikram{check} 
...
%
This turnaround time depends on whether the job needs to be recomputed because of preemptions, and is given by Equation~\ref{eqn:10}.
